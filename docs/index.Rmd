...
|
| **[A]** The Connectivity-Based Outlier Factor algorithm was implemented only for the <span style="color: #FF0000">PC1</span> and <span style="color: #FF0000">PC2</span> principal component descriptors using the <mark style="background-color: #CCECFF">**DDoutlier**</mark> package.  
|
| **[B]** The algorithm contains 1 hyperparameter:
|      **[B.1]** <span style="color: #FF0000">ndims</span> = The number of k-nearest neighbors to construct a set-based nearest-path with and the number of neighbors for each observation to compare chaining-distance with, held constant at a value of 200.
|
...
ggplot(OD_COF_Summary, aes(x=PC1, y=PC2, color=Scores, size= Scores)) +
  geom_point() +
  scale_color_gradient(low="#0000AA55", high="#AA000055") +
  theme_bw() +
  facet_grid(. ~ Label) +
...
###  1.5.5. Aggregated K-Nearest Neighbors Distance (KNN-AGG)

###  1.5.6. In-Degree for Observations in a K-Nearest Neighbors Graph (KNN-IN)
