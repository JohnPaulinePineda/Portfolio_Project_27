...
# by outlier classes
##################################
ggplot(OD_DB_Summary, aes(x=PC1, y=PC2, color=Class)) +
...
# adjusted to the outlier scores
##################################
ggplot(OD_LOF_Summary, aes(x=PC1, y=PC2, color=Scores, size= Scores)) +
  geom_point() +
  scale_color_gradient(low="#0000AA1A", high="#AA00001A") +
  theme_bw() +
  facet_grid(. ~ Label) +
...
|
| **[A]** The LOOP algorithm was implemented only for the <span style="color: #FF0000">PC1</span> and <span style="color: #FF0000">PC2</span> principal component descriptors using the <mark style="background-color: #CCECFF">**DDoutlier**</mark> package.  
|
| **[B]** The algorithm contains 2 hyperparameterS:
|      **[B.1]** <span style="color: #FF0000">k</span> = number of k-nearest neighbors to compare density with, held constant at a value of 5.
|      **[B.2]** <span style="color: #FF0000">lambda</span> = multiplication factor for standard deviation, held constant at a value of 1.
|
| **[C]** Using the label information from the <span style="color: #FF0000">Status</span> variable defined prior to the analysis, the algorithm was not able to relatively discriminate between <span style="color: #FF0000">Status=Outlier</span> and <span style="color: #FF0000">Status=Valid</span>, as demonstrated by their differentially expressed outlier score densities.
|      **[C.1]** ROC Curve AUC = 0.XXXX
|
...
##################################
# Implementing the LOOP Algorithm
##################################
OD_LOOP <- LOOP(OD, k=5, lambda=1)
OD_LOOP <- ifelse(OD_LOOP==1,"Outlier","Inlier")
...
# applied to the dataset
##################################
OD_LOOP_PredictedClass <- OD_LOOP

##################################
# Consolidating the outlier classes
##################################
OD_LOOP_Summary <- DR_PCA_SUBSET
OD_LOOP_Summary$Class <- OD_LOOP_PredictedClass
OD_LOOP_Summary$Status <- as.character(OD_LOOP_Summary$Status)
OD_LOOP_Summary$Label <- rep("LOOP", nrow(OD_LOOP_Summary))
...
# by outlier classes
##################################
ggplot(OD_LOOP_Summary, aes(x=PC1, y=PC2, color=Class)) +
  geom_point(size=3, alpha=0.10) +
  scale_color_manual(values=c("#0000AA", "#AA0000")) +
  theme_bw() +
  facet_grid(. ~ Label) +
  ...
  theme(legend.position="top",
        plot.title=element_text(color="black",size=15,face="bold",hjust=0.50)) +
  ggtitle("Outlier Classes : Local Outlier Probability (LOOP)")

##################################
# Exploring the outlier classes
# between the valid and outlier points
##################################
Class_Status_Data <- as.data.frame(cbind(OD_LOOP_Summary$Class,
                                         OD_LOOP_Summary$Status))
colnames(Class_Status_Data) <- c("Class","Status")
Class_Status_Proportion <- as.data.frame(prop.table(table(Class_Status_Data), 2))
Class_Status_Proportion$Label <- rep("LOOP", nrow(Class_Status_Proportion))
Class_Status_Proportion
...
barchart(Freq ~ Status | Label,
         ...
         stack = TRUE,
         ylab = "Proportion",
         main = "Outlier Classes : Local Correlation Integral (LOOP)",
         xlab = "Status",
         auto.key = list(adj = 1))
...
##################################
Status <- ifelse(OD_LOOP_Summary$Status=="Valid",0,1)
OD_LOOP_PredictedClassLevel <- ifelse(OD_LOOP_Summary$Class=="Inlier",0,1)

(OD_LOOP_ROCCurveAUC <- AUC(OD_LOOP_PredictedClassLevel, Status))

```

###  1.5.13. Natural Outlier Factor (NOF)
|
...
# adjusted to the outlier scores
##################################
ggplot(OD_NOF_Summary, aes(x=PC1, y=PC2, color=Scores, size= Scores)) +
  geom_point() +
  scale_color_gradient(low="#0000AA1A", high="#AA00001A") +
  theme_bw() +
  facet_grid(. ~ Label) +
...
###  1.5.14. Relative Density-Based Outlier Factor (RDOS)
|
| **[A]** The RDOS algorithm was implemented only for the <span style="color: #FF0000">PC1</span> and <span style="color: #FF0000">PC2</span> principal component descriptors using the <mark style="background-color: #CCECFF">**DDoutlier**</mark> package.  
|
| **[B]** The algorithm contains 2 hyperparameters:
|      **[B.1]** <span style="color: #FF0000">k</span> = number of k-nearest neighbors used to identify reverse- and shared nearest neighbors, held constant at a value of 50.
|      **[B.2]** <span style="color: #FF0000">h</span> = bandwidth parameter for gaussian kernel held constant at a value of 0.10.
|
| **[C]** Using the label information from the <span style="color: #FF0000">Status</span> variable defined prior to the analysis, the algorithm was able to relatively discriminate between <span style="color: #FF0000">Status=Outlier</span> and <span style="color: #FF0000">Status=Valid</span>, as demonstrated by their differentially expressed outlier score densities.
|      **[C.1]** ROC Curve AUC = 0.xxxx
|
...
##################################
# Implementing the RDOS Algorithm
##################################
OD_RDOS <- RDOS(OD, k=20, h=2)
...
# applied to the dataset
##################################
OD_RDOS_PredictedScores <- OD_RDOS

##################################
# Exploring the outlier scores
# between the valid and outlier points
##################################
max(OD_RDOS_PredictedScores)
min(OD_RDOS_PredictedScores)

##################################
# Consolidating the outlier scores
##################################
OD_RDOS_Summary <- DR_PCA_SUBSET
OD_RDOS_Summary$Scores <- OD_RDOS_PredictedScores
OD_RDOS_Summary$Label <- rep("RDOS", nrow(OD_RDOS_Summary))
...
# adjusted to the outlier scores
##################################
ggplot(OD_RDOS_Summary, aes(x=PC1, y=PC2, color=Scores, size= Scores)) +
  geom_point() +
  scale_color_gradient(low="#0000AA1A", high="#AA00001A") +
  theme_bw() +
  facet_grid(. ~ Label) +
  ...
  theme(legend.position="top",
        plot.title=element_text(color="black",size=15,face="bold",hjust=0.50)) +
  ggtitle("Outlier Scores : Relative Density-Based Outlier Factor (RDOS)")
...
##################################
densityplot( ~ Scores | Label,
             data = OD_RDOS_Summary,
             groups = Status,
             xlab = "Outlier Scores",
             ylab = "Density",
             auto.key = list(columns = (length(levels(OD_RDOS_Summary$Status)))))

##################################
# Evaluating the apparent 
# discrimination performance
# of the RDOS Algorithm
##################################
Status <- ifelse(OD_RDOS_Summary$Status=="Valid",0,1)

(OD_RDOS_ROCCurveAUC <- AUC(OD_RDOS_PredictedScores, Status))

```

###  1.5.15. Robust Kernel-Based Outlier Factor (RKOF)
|
| **[A]** The RKOF algorithm was implemented only for the <span style="color: #FF0000">PC1</span> and <span style="color: #FF0000">PC2</span> principal component descriptors using the <mark style="background-color: #CCECFF">**DDoutlier**</mark> package.  
|
| **[B]** The algorithm contains 4 hyperparameterS:
|      **[B.1]** <span style="color: #FF0000">k</span> = number of nearest neighbors to compare density estimation with, held constant at a value of 50.
|      **[B.2]** <span style="color: #FF0000">C</span> = mMultiplication parameter for k-distance of neighboring observations held constant at a value of 1.
|      **[B.3]** <span style="color: #FF0000">alpha</span> = sensivity parameter for k-distance and bandwidth, held constant at a value of 1.
|      **[B.4]** <span style="color: #FF0000">sigma2</span> = variance parameter for weighting of neighboring observations held constant at a value of 1.
|
| **[C]** Using the label information from the <span style="color: #FF0000">Status</span> variable defined prior to the analysis, the algorithm was able to relatively discriminate between <span style="color: #FF0000">Status=Outlier</span> and <span style="color: #FF0000">Status=Valid</span>, as demonstrated by their differentially expressed outlier score densities.
|      **[C.1]** ROC Curve AUC = 0.xxxx
|
...
##################################
# Implementing the RKOF Algorithm
##################################
OD_RKOF <- RKOF(OD, k=50, C=1, alpha=1, sigma2=1)
...
# applied to the dataset
##################################
OD_RKOF_PredictedScores <- OD_RKOF

##################################
# Exploring the outlier scores
# between the valid and outlier points
##################################
max(OD_RKOF_PredictedScores)
min(OD_RKOF_PredictedScores)

##################################
# Consolidating the outlier scores
##################################
OD_RKOF_Summary <- DR_PCA_SUBSET
OD_RKOF_Summary$Scores <- OD_RKOF_PredictedScores
OD_RKOF_Summary$Label <- rep("RKOF", nrow(OD_RKOF_Summary))
...
# adjusted to the outlier scores
##################################
ggplot(OD_RKOF_Summary, aes(x=PC1, y=PC2, color=Scores, size= Scores)) +
  geom_point() +
  scale_color_gradient(low="#0000AA1A", high="#AA00001A") +
  theme_bw() +
  facet_grid(. ~ Label) +
  ...
  theme(legend.position="top",
        plot.title=element_text(color="black",size=15,face="bold",hjust=0.50)) +
  ggtitle("Outlier Scores : Robust Kernel-Based Outlier Factor (RKOF)")
...
##################################
densityplot( ~ Scores | Label,
             data = OD_RKOF_Summary,
             groups = Status,
             xlab = "Outlier Scores",
             ylab = "Density",
             auto.key = list(columns = (length(levels(OD_RKOF_Summary$Status)))))

##################################
# Evaluating the apparent 
# discrimination performance
# of the RKOF Algorithm
##################################
Status <- ifelse(OD_RKOF_Summary$Status=="Valid",0,1)

(OD_RKOF_ROCCurveAUC <- AUC(OD_RKOF_PredictedScores, Status))
